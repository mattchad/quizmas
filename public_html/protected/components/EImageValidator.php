...
class EImageValidator extends CFileValidator {
    ...
    protected function validateAttribute($object, $attribute) {
        ...
        if (isset($this->minWidth, $this->minHeight) && $data !== false) {
            if ($data[0] < $this->minWidth && $data[1] < $this->minHeight) {
                $message = Yii::t('yii', 'This image should be at least {minWidth} pixels wide and {minHeight} pixels high');
                $this->addError($object, $attribute, $message, array('{minWidth}' => $this->minWidth, '{minHeight}' => $this->minHeight));
                return;
            }
        }
    }
